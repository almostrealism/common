...
if [ "${FORCE_REBUILD}" = true ] || [ ! -d "${DEPS_DIR}" ] || [ -z "$(ls -A "${DEPS_DIR}" 2>/dev/null)" ]; then
    ...
    mvn -f "${PROJECT_ROOT}/pom.xml" dependency:copy-dependencies \
        -pl flowtree/core -DoutputDirectory=flowtree/core/target/dependency -q
fi
