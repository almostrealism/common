...
AGENT_ENV="flowtree/core/agent/.env"

# ── Usage ──────────────────────────────────────────────────────────
usage() {
  cat <<'EOF'
Usage: flowtree/core/rebuild.sh [services...] [--agents]
...
Examples:
  flowtree/core/rebuild.sh                        # controller stack only
  flowtree/core/rebuild.sh --agents               # controller stack + agents
  flowtree/core/rebuild.sh --agents-only          # agents only
  flowtree/core/rebuild.sh flowtree-controller    # just the controller service
EOF
  exit 0
}
...
if [ "${NEEDS_BUILD}" = true ]; then
  ...
  mvn install -pl flowtree/core -am -DskipTests
...
if [ "${AGENTS_ONLY}" = false ]; then
  if [ ${#SERVICES[@]} -eq 0 ]; then
    ...
    docker compose -f flowtree/core/controller/docker-compose.yml build --no-cache --pull
    docker compose -f flowtree/core/controller/docker-compose.yml up -d --force-recreate
  else
    echo "Rebuilding (no cache): ${SERVICES[*]}"
    docker compose -f flowtree/core/controller/docker-compose.yml build --no-cache --pull "${SERVICES[@]}"
    docker compose -f flowtree/core/controller/docker-compose.yml up -d --force-recreate "${SERVICES[@]}"
  fi
fi
...
if [ "${AGENTS}" = true ] || [ "${AGENTS_ONLY}" = true ]; then
  ...
  docker compose -f flowtree/core/agent/docker-compose.yml build --no-cache --pull
  docker compose -f flowtree/core/agent/docker-compose.yml up -d --force-recreate
fi
