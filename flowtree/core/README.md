...
```
  ...
  (Slack, CI, API)
         ...
         ▼
┌────────────────────────────────────────────────────────┐
│  Controller Stack  (flowtree/core/controller/docker-compose) │
│                                                        │
│  ┌─────────────────────┐    ┌──────────────────────┐  │
...
└─────────────┼──────────────────────────────────────────┘
              ...
              ▼
┌────────────────────────────────────────────────────────┐
│  Agent Pool  (flowtree/core/agent/docker-compose)           │
│                                                        │
│  ┌───────────┐   ┌───────────┐   ┌───────────┐        │
...
|-----------|----------|------|
| `flowtree-controller` | `flowtree/core/controller/Dockerfile` | Java server that accepts agent connections (:7766), exposes a low-level job submission API (:7780), integrates with Slack, and dispatches `ClaudeCodeJob` instances to agents |
...
| Agent containers | `flowtree/core/agent/` | Each agent connects outbound to the controller, receives `ClaudeCodeJob` instances, and executes Claude Code prompts inside a Docker container with its own git workspace |
...
```bash
./flowtree/core/rebuild.sh flowtree-controller   # just the controller
./flowtree/core/rebuild.sh ar-manager            # just the manager
```

### Start the agent pool too

```bash
./flowtree/core/rebuild.sh --agents        # controller stack + agent pool
./flowtree/core/rebuild.sh --agents-only   # agent pool only (controller already running)
```
...
# View logs
docker compose -f flowtree/core/controller/docker-compose.yml logs -f flowtree-controller

# Stop
docker compose -f flowtree/core/controller/docker-compose.yml down
```

### Workstream configuration

Workstreams are defined in `/Users/Shared/flowtree/controller/workstreams.yaml`. Each workstream maps a Slack channel (or API endpoint) to a git repository and branch. See `flowtree/core/src/main/resources/workstreams-example.yaml` for the full reference.
...
```bash
# Docker (recommended)
./flowtree/core/rebuild.sh --agents-only
