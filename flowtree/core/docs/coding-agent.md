...
Convenience scripts live in `flowtree/core/bin/` and use `mvn exec:java` for classpath resolution. Each script auto-builds the module on first run if the `target/` directory is missing.
...
./flowtree/core/bin/start-controller.sh --channel C0123ABCDEF

# Using a tokens file
./flowtree/core/bin/start-controller.sh --tokens slack-tokens.json --config workstreams.yaml

# Custom FlowTree port
./flowtree/core/bin/start-controller.sh --flowtree-port 8800 --config workstreams.yaml

# Show full usage
./flowtree/core/bin/start-controller.sh --help
```
...
./flowtree/core/bin/start-agent.sh

# Connect to a specific controller
FLOWTREE_ROOT_HOST=10.0.0.1 FLOWTREE_ROOT_PORT=7766 ./flowtree/core/bin/start-agent.sh
```
