...
/**
 * Fields, accessors, and the {@link #toWorkstream()} conversion for a single
 * workstream's persisted configuration entry.
 *
 * <p>Extracted from {@link WorkstreamConfig.WorkstreamEntry}, which now
 * extends this class as an empty subclass, to keep {@code WorkstreamConfig}
 * within the project's file-length limit. {@link WorkstreamConfig.WorkstreamEntry}
 * remains the type callers use; this class exists purely so that type's
 * fully-qualified name and default constructor are unaffected by the split.
 */
@JsonIgnoreProperties(ignoreUnknown = true)
public class WorkstreamEntryData {
    ...
    /**
     * Optional natural-language record of the intent that originally seeded this
     * workstream. See {@link Workstream#getPlanInstructions()} for the full
     * semantics — this is a record of how the work began, not a specification
     * the branch must conform to.
     */
    ...
    /**
     * Returns the optional natural-language record of the intent that
     * originally seeded this workstream, or {@code null}.
     */
    public String getPlanInstructions() { return planInstructions; }
    /** Sets the recorded original-intent instructions for this workstream. */
    public void setPlanInstructions(String planInstructions) { this.planInstructions = planInstructions; }
     ...
     *
     * <p>The {@link PhaseConfigBundle} is applied last and layered on top of
     * the legacy {@code defaultRunner}/{@code runners} values set just before
     * it: the bundle setter rewrites the runner-resolution fields, so any
     * runner present in the bundle wins over the legacy values, matching the
     * "new wins" precedence used when loading YAML.</p>
     */
    public Workstream toWorkstream() {
        ...
        ws.setPlanInstructions(planInstructions);
