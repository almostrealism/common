...
| `allowedTools` | `Read,Edit,Write,Bash,Glob,Grep,TaskOutput,TaskStop` | Comma-separated tool allowlist (MCP tools are appended automatically) |
...
Convenience scripts live in `flowtree/runtime/bin/` and use `mvn exec:java` for classpath resolution. Each script auto-builds the module on first run if the `target/` directory is missing.
...
./flowtree/runtime/bin/start-controller.sh --channel C0123ABCDEF

# Using a tokens file
./flowtree/runtime/bin/start-controller.sh --tokens slack-tokens.json --config workstreams.yaml

# Custom FlowTree port
./flowtree/runtime/bin/start-controller.sh --flowtree-port 8800 --config workstreams.yaml

# Show full usage
./flowtree/runtime/bin/start-controller.sh --help
```
...
FLOWTREE_ROOT_HOST=10.0.0.1 FLOWTREE_ROOT_PORT=7766 ./flowtree/runtime/bin/start-agent.sh
```
