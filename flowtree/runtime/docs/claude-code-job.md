...
The `DEFAULT_TOOLS` constant defines the minimum tool set that every Claude Code session receives: basic file system access (Read, Write, Edit, Glob, Grep), shell command execution (Bash), and control of a background task the session started (TaskOutput reads one's output back, TaskStop ends one). Without the first six the agent could not perform any useful work; without the last two it can start a long-running command in the background and then neither read its result nor stop it, because a tool absent from this list is refused by the harness before any hook sees it. Additional tools are appended by the MCP config builder, including MCP server tools (prefixed with `mcp__<serverName>__`) and any custom tools defined in the workstream configuration.
...
**Protected Test Files** -- When `protectTestFiles` is true, files matching `**/src/test/**` or `**/src/it/**` are checked against the merge-base of `origin/<baseBranch>` and `HEAD` (not the base branch's live tip). A file absent at the merge-base is a branch-new file and is allowed through in full. For any non-`.java` file, protection is whole-file: existing at the merge-base blocks it outright. CI configuration (`.github/workflows/**`, `.github/actions/**`, `tools/ci/**`) is not covered by this lock but by the separate CI file lock (`protectCiFiles`), which blocks a CI file whole-file whether or not it exists at the merge-base. For a `.java` file it is test-method-level (`TestMethodProtection`, which shells out to the shared `tools/ci/agent-protection/test-method-lines.awk`): the file is blocked only if an existing `@Test` method's content changed (even by addition, e.g. an inserted early return) or was removed; new test methods, and edits to fixtures, helpers, fields, or methods the branch itself introduced, are all allowed. See [file-staging.md](file-staging.md#guardrail-2-test-file-protection) for the full algorithm. Every existence/content check fails safe (blocks on error).
...
MCP servers come from three distinct sources, processed in priority order:

#### 1. The ar-manager HTTP entry

`ar-manager` is the single centralized HTTP MCP server. When `arManagerUrl` and `arManagerToken` are both set on the builder (the default for any job dispatched by the controller), `buildMcpConfig()` emits a `type: http` entry with the bearer token in the `Authorization` header. Its allowlist (`McpConfigBuilder.AR_MANAGER_TOOL_NAMES`) carries every tool currently exposed to agents: `send_message`, the `github_pr_*` family, the read-only `tracker_*` queries, `memory_recall`/`memory_store`, `workstream_*`, `controller_health`, and `project_read_plan`.
...
**Output in --allowedTools:** Each name in `AR_MANAGER_TOOL_NAMES` is added as `mcp__ar-manager__<toolName>`.
...
**Discovery:** `discoverProjectMcpServers()` reads `.mcp.json` from the working directory, parses each server's command and args, extracts the Python source file path (first arg), and cross-references with `.claude/settings.json`'s `enabledMcpjsonServers` array. The `ar-manager` server is always skipped (handled separately by the HTTP entry). Servers that are already centralized or pushed are also skipped.
...
1. The ar-manager allowlist: every name in `AR_MANAGER_TOOL_NAMES` rendered as `mcp__ar-manager__<toolName>`, but only when both the URL and bearer token are set.
2. Tools from any other centralized servers, same `mcp__<serverName>__<toolName>` format.
3. Tools from pushed tools, same format.
4. Tools from discovered project servers: for each server, `discoverToolNames()` is called on the Python source file and the resulting names are formatted as `mcp__<serverName>__<toolName>`.
...
3. **Filter exclusions**: The `ar-manager` server is always excluded (it has its own HTTP entry). Servers that appear in the centralized config or pushed tools config are also excluded (they are handled by their respective higher-priority paths).
...
The configuration sources are processed with strict deduplication to prevent the same server from appearing twice in the MCP config or allowed-tools list:

1. The `ar-manager` HTTP entry takes highest priority.
2. Other centralized servers (if any are configured) take second priority. They appear as HTTP entries.
3. Pushed tools take third priority. They appear as stdio entries.
4. Project servers take fourth priority. They are skipped if the server name already appears as `ar-manager` or in the centralized or pushed configs.
...
The full download URL has the form `http://0.0.0.0:<port>/api/tools/<name>`, where `<port>` is the `FlowTreeApiEndpoint` listening port (default: 7780) and `<name>` is the server name as declared in `workstreams.yaml`. The `0.0.0.0` placeholder is resolved to the actual controller host before the download request is made.
...
- `centralMcp` = `{}` (the ar-manager HTTP entry is configured via the dedicated `arManagerUrl`/`arManagerToken` fields rather than this generic centralized-config map)
...
|---|---|---|
| `AR_WORKSTREAM_URL` | `resolveWorkstreamUrl()` | The workstream URL with `0.0.0.0` replaced by the controller's actual host. Forwarded to the controller alongside the workstream id so ar-manager's `send_message` tool can route updates to the correct workstream and so in-container helpers (ar-secrets) can call the controller's REST API. |
| `AR_CONTROLLER_URL` | `extractControllerBaseUrl(wsUrl)` | The scheme+authority portion of `AR_WORKSTREAM_URL`. Read by ar-secrets to reach `/api/secrets/*`. |
| `AR_WORKSTREAM_ID` | `extractWorkstreamId(wsUrl)` | The workstream identifier embedded in `AR_WORKSTREAM_URL`. Sent as the `workstream_id` query parameter on secret API calls. |
| `AR_MANAGER_TOKEN` | `arManagerToken` | The `armt_tmp_*` bearer token issued at job dispatch. Reused by ar-secrets as the bearer on its controller calls. |
