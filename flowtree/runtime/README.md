...
```
  ...
  (Slack, CI, API)
         ...
         ▼
┌────────────────────────────────────────────────────────┐
│  Controller Stack  (flowtree/runtime/controller/docker-compose) │
│                                                        │
│  ┌─────────────────────┐    ┌──────────────────────┐  │
...
└─────────────┼──────────────────────────────────────────┘
              ...
              ▼
┌────────────────────────────────────────────────────────┐
│  Agent Pool  (flowtree/runtime/agent/docker-compose)           │
│                                                        │
│  ┌───────────┐   ┌───────────┐   ┌───────────┐        │
...
```bash
./flowtree/runtime/rebuild.sh flowtree-controller   # just the controller
./flowtree/runtime/rebuild.sh ar-manager            # just the manager
```
...
# View logs
docker compose -f flowtree/runtime/controller/docker-compose.yml logs -f flowtree-controller

# Stop
docker compose -f flowtree/runtime/controller/docker-compose.yml down
```

### Workstream configuration

Workstreams are defined in `/Users/Shared/flowtree/controller/workstreams.yaml`. Each workstream maps a Slack channel (or API endpoint) to a git repository and branch. See `flowtree/runtime/src/main/resources/workstreams-example.yaml` for the full reference.
...
```bash
# Docker (recommended)
./flowtree/runtime/rebuild.sh --agents-only
...
# first time only: it stops and prints two `sudo` commands that register the
# service; run them as an administrator, then run install.sh again
```
