...
6. Log the agent containers in to Claude. `ClaudeCodeRunner` launches the
   agent under a real pty (see
   [../architecture/AGENT_RUNNERS.md § Subprocess launch backends](../architecture/AGENT_RUNNERS.md#subprocess-launch-backends))
   and that mode requires interactively cached credentials, not a
   long-lived headless token. For each agent container:
