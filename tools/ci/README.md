...
| Script | Purpose |
|---|---|
| `fetch-latest-coverage.sh` | Reuse (or, with `FORCE=true`, recompute) the merged JaCoCo report and a fresh coverage.py report; `ALLOW_RECOMPUTE=false` makes a needed recompute fail fast instead |
