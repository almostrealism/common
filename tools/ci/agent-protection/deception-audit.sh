...
# `set -e`. Agents are told to run this check locally before staging, and
# they run on macOS hosts, so it has to work under the oldest bash any of
# them provides rather than only under the CI runner's.
MERGE_BASE_FILE_SET="
