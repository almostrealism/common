...
# Launched as a script (Dockerfile ENTRYPOINT, start.sh) this file is the
# module `__main__`, while the tool modules at the bottom reach back for it as
# `server`. Without the alias Python runs the file a second time under that
# name and those modules decorate the second copy's FastMCP instance.
...
from prompt_linting import (  # noqa: E402
    _COMMIT_LINTER_MIN_LEN,
...
#
# Every tool but the three above is registered here, so nothing may serve
# before these have run — hence `main` being invoked below rather than where
# it is defined.
# ---------------------------------------------------------------------------
...
from workstream_submit_tools import (  # noqa: E402
    workstream_submit_task,
)
...
# Startup lives in its own module and is imported here, last of all, so that
# `main` cannot be reached until every tool above has registered.
from startup import main  # noqa: E402
