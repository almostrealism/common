...
_CMD_PREFIXES = {"!", "time", "nohup", "sudo", "command", "exec", "builtin", "stdbuf", "nice",
                 "ionice", "timeout"}
...
# _CMD_PREFIXES wrappers that take positional operands of their own after
# their option flags and before the wrapped command -- ``timeout DURATION
# mvn test`` -- keyed by the wrapper's base name to the number of such
# operands. Without this, ``timeout 2400 mvn test`` would leave "2400" as the
# wrapped command's apparent first token and never reach "mvn".
_CMD_PREFIX_POSITIONAL_OPERANDS = {"timeout": 1}
...
def _unwrap_cmd_prefix_options(wrapper_base: str, tokens: list) -> list:
    ...
    then skips the wrapper's own positional operands listed in
    ``_CMD_PREFIX_POSITIONAL_OPERANDS`` (the DURATION of ``timeout 2400 mvn
    test``).
    """
    ...
    if i < len(tokens) and tokens[i] == "--":
        i += 1
    i += _CMD_PREFIX_POSITIONAL_OPERANDS.get(wrapper_base, 0)
    return tokens[i:]
