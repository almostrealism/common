...
The substantive test classes live in `test_workspace_tools.py` at the top of the
...
`test_workspace_tools.py`, not here. Adding a new workspace-related TestCase
to `test_workspace_tools.py` does not require an edit here as long as its name is
...
import test_workspace_tools  # noqa: E402  -- sys.path adjusted above

_WORKSPACE_CLASSES = [
    name
    for name, obj in vars(test_workspace_tools).items()
...
if not _WORKSPACE_CLASSES:
    raise RuntimeError(
        "No workspace TestCase classes found in test_workspace_tools.py — "
        "the re-export in tests/test_workspace_config.py is broken."
    )

for _name in _WORKSPACE_CLASSES:
    globals()[_name] = getattr(test_workspace_tools, _name)
