...
- `--port 8084` -- the default port the consultant checks
...
2. The consultant auto-detects `mac-studio:8084` as a fallback when `localhost:8084` is unreachable

To explicitly set the URL:
```bash
export AR_CONSULTANT_LLAMACPP_URL=http://mac-studio:8084
```
...
- `nc -z mac-studio 8084` succeeds (TCP connects)
- `curl http://mac-studio:8084/health` hangs and eventually times out
- `curl http://localhost:8084/health` works on the server machine itself
...
```bash
export AR_CONSULTANT_LLAMACPP_URL=http://host.docker.internal:8084
```
...
| `AR_CONSULTANT_LLAMACPP_URL` | `localhost:8084` (host) / `host.docker.internal:8084` (container) | llama.cpp server URL |
...
2. If using Docker: `docker compose -f flowtree/runtime/controller/docker-compose.yml ps ar-memory`
