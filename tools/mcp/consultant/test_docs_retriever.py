...
appear in standalone (non-Maven-layer) docs — flowtree/docs, tools/mcp,
...
class TestStandaloneDocCoverage(unittest.TestCase):
    ...
    def test_pushed_tools_topic_is_indexed(self):
        self._assertHasHitIn("registerPushedTools", "flowtree/docs/")

    def test_managed_tools_downloader_is_indexed(self):
        self._assertHasHitIn("ManagedToolsDownloader", "flowtree/docs/")

    def test_armt_tmp_token_is_indexed(self):
        self._assertHasHitIn("armt_tmp", "flowtree/docs/")

    def test_secret_render_file_is_indexed(self):
        # Hit may come from flowtree/docs or tools/mcp — either is fine.
        hits = self.retriever.search("secret_render_file", max_results=5)
        self.assertTrue(
            any(
                h["file"].startswith("flowtree/docs/")
                or h["file"].startswith("tools/mcp/")
                for h in hits
            ),
            f"Expected a hit in flowtree/docs/ or tools/mcp/; got "
...
class TestFileLists(unittest.TestCase):
    ...
    def test_markdown_files_includes_flowtree_docs(self):
        ...
        self.assertIn("flowtree/docs/architecture.md", files)
        ...
        self.assertIn("flowtree/README.md", readme_files)
    ...
    def test_all_doc_files_includes_standalone_roots(self):
        ...
        self.assertIn("flowtree/docs/claude-code-job.md", files)
        self.assertIn("docs/tutorials/09-workspace-secrets.html", files)
