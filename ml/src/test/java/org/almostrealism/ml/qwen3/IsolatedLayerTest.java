...
/**
 * Test individual layers in isolation using previous layer's PyTorch output.
 */
public class IsolatedLayerTest implements AttentionFeatures, ConsoleFeatures {
    ...
    /**
     * Test layers 0, 10, 20, 22, 23 in isolation to see if the bug is layer-specific.
     */
    @Test
    public void testMultipleLayersIsolated() throws Exception {
        String logFile = "/workspace/project/common/ml/test_output/isolated_layers.txt";
        ...
        int[] layersToTest = {0, 5, 10, 15, 20, 22, 23};

        log(String.format("%-10s %-15s %-20s %-20s %-15s", "Layer", "MAE", "PyTorch Range", "AR Range", "Ratio"));
        log("-".repeat(85));

        for (int layerIdx : layersToTest) {
            // Load input (previous layer's output or embeddings for layer 0)
            float[] inputData;
            if (layerIdx == 0) {
                // For layer 0, use embeddings
                PackedCollection embeddings = stateDict.get("model.embed_tokens.weight");
                int tokenId = 9707;
                inputData = new float[config.dim];
                for (int i = 0; i < config.dim; i++) {
                    inputData[i] = (float) embeddings.toDouble(tokenId * config.dim + i);
                }
            } else {
                inputData = loadReferenceOutput("after_layer_" + (layerIdx - 1) + ".bin");
            }
            ...
            // Build single layer
            Model singleLayerModel = new Model(shape(1, config.dim));
            ...
            singleLayerModel.add(transformer(
            ...
            PackedCollection output = singleLayerModel.compile().forward(input);
            ...
            // Compute error and ranges
            double mae = 0;
            for (int i = 0; i < config.dim; i++) {
                mae += Math.abs(pytorchOutput[i] - arOutput[i]);
            }
            mae /= config.dim;

            float pyMin = min(pytorchOutput), pyMax = max(pytorchOutput);
            float arMin = min(arOutput), arMax = max(arOutput);

            // Compute average ratio (how much smaller AR is vs PyTorch)
            double ratio = (Math.abs(pyMax) + Math.abs(pyMin)) / (Math.abs(arMax) + Math.abs(arMin) + 1e-10);

            String pyRange = String.format("[%.2f, %.2f]", pyMin, pyMax);
            String arRange = String.format("[%.2f, %.2f]", arMin, arMax);

            log(String.format("%-10d %-15.4f %-20s %-20s %-15.2f",
                layerIdx, mae, pyRange, arRange, ratio));
        }
