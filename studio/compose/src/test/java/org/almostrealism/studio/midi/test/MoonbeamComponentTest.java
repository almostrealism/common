...
public class MoonbeamComponentTest extends TestSuiteBase implements ConsoleFeatures {
	 ...
	 * translation bias, and linear projection. The value is a kernel argument and the
	 * producer passes through the process optimizer, as it does for the generator: an
	 * embedding built with the value as a literal is a different program per value, and
	 * fused into one kernel it takes longer to compile on OpenCL than this test allows.
	 */
	@Test(timeout = 5_000)
	public void testSingleFmeEmbed() {
		...
		PackedCollection result = Process.optimized(fme.embed(cp(PackedCollection.of(42.0)))).get().evaluate();
	 ...
	 * and the concatenation to produce the full hidden-size vector. The token's values
	 * are a kernel argument and the producer passes through the process optimizer, as
	 * for the generator; see {@link #testSingleFmeEmbed()}.
	 */
	@Test(timeout = 10_000)
	public void testSingleCompoundEmbedding() {
		...
		long computeStart = System.currentTimeMillis();
		PackedCollection result = Process.optimized(embedding.embedValues(cp(token.pack()))).get().evaluate();
		long computeTime = System.currentTimeMillis() - computeStart;
		...
		long elapsed = System.currentTimeMillis() - start;
		log("testSingleCompoundEmbedding: "
				+ elapsed + " ms (compute: " + computeTime + " ms)");
	}
	...
	public void testRopeFrequencyComputation() {
		...
		for (int g = 0; g < REAL_CONFIG.ropeThetas.length; g++) {
			...
			log("freqCis theta=" + theta
					+ " (headDim=" + headDim + ", seqLen=" + maxSeqLen
					+ "): " + groupTime + " ms");
		}

		long elapsed = System.currentTimeMillis() - start;
		log("testRopeFrequencyComputation: "
				+ elapsed + " ms");
	}
	...
	public void testSingleGruBlock() {
		...
		log("GRU block weight alloc: " + allocTime + " ms");
	...
	public void testGruDecoderRealDims() {
		...
		GRUDecoder decoder = createRandomDecoder(REAL_CONFIG);
		long allocTime = System.currentTimeMillis() - allocStart;
		log("GRU decoder alloc: " + allocTime + " ms");
		...
		log("GRU decode tokens: "
				+ Arrays.toString(tokens));

		long elapsed = System.currentTimeMillis() - start;
		log("testGruDecoderRealDims: "
	...
	public void testSingleAttentionLayerBuild() {
		...
		long allocTime = System.currentTimeMillis() - allocStart;
		log("1-layer weight alloc: " + allocTime + " ms");
		...
		long elapsed = System.currentTimeMillis() - start;
		log("testSingleAttentionLayerBuild: "
	...
	public void testSingleTransformerBlockForward() {
		...
		MoonbeamMidi model = new MoonbeamMidi(oneLayerConfig, stateDict, embedding, decoder);
		long buildTime = System.currentTimeMillis() - buildStart;
		log("1-layer model build: " + buildTime + " ms");
		...
		long elapsed = System.currentTimeMillis() - start;
		log("testSingleTransformerBlockForward: "
	...
	public void testTwoLayerForwardPass() {
		...
		log("2-layer generate: " + genTime + " ms");

		Assert.assertNotNull("Generated token should not be null", generated);

		long elapsed = System.currentTimeMillis() - start;
		log("testTwoLayerForwardPass: " + elapsed + " ms");
	}
	...
	public void testFullModelForwardPass() {
		...
		GRUDecoder decoder = createRandomDecoder(REAL_CONFIG);
		long allocTime = System.currentTimeMillis() - allocStart;
		log("15-layer weight alloc: " + allocTime + " ms");

		long buildStart = System.currentTimeMillis();
		MoonbeamMidi model = new MoonbeamMidi(REAL_CONFIG, stateDict, embedding, decoder);
		long buildTime = System.currentTimeMillis() - buildStart;
		log("15-layer model build: " + buildTime + " ms");
		...
		Assert.assertNotNull("Forward should produce a token", result);
		log("15-layer forward+decode: " + fwdTime + " ms");

		long elapsed = System.currentTimeMillis() - start;
		log("testFullModelForwardPass: " + elapsed + " ms");
	}
	...
	public void testSingleAutoregressiveStep() {
		...
		GRUDecoder decoder = createRandomDecoder(REAL_CONFIG);
		long allocTime = System.currentTimeMillis() - allocStart;
		log("weight alloc: " + allocTime + " ms");

		long buildStart = System.currentTimeMillis();
		MoonbeamMidi model = new MoonbeamMidi(REAL_CONFIG, stateDict, embedding, decoder);
		long buildTime = System.currentTimeMillis() - buildStart;
		log("model build: " + buildTime + " ms");
		...
		log("autoregressive step: " + stepTime + " ms");
		log("generated token: onset="
		...
		long elapsed = System.currentTimeMillis() - start;
		log("testSingleAutoregressiveStep: " + elapsed + " ms");
	}
