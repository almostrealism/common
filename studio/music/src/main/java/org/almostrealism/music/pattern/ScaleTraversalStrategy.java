...
public enum ScaleTraversalStrategy implements CodeFeatures, ConsoleFeatures {
	...
	private RenderedNoteAudio createRenderedNote(PatternElement element,
		...
		populateBatchedInputs(note, element, details, automationLevel,
				audioContext, durationSec);
		return note;
	}

	/**
	 * Populates the per-note batched-input fields on the {@link RenderedNoteAudio}
	 * so the Phase 3 batched dispatch path (see
	 * {@link BatchedPatternLayerRenderer#render}) can call
	 * {@link org.almostrealism.audio.BatchedPatternRenderer#buildBatchedChain}
	 * instead of falling back to {@link PatternFeatures#renderPerNote}.
	 *
	 * <p>The data is extracted from the existing producer chain at note
	 * construction time:</p>
	 * <ul>
	 *   <li><b>Source</b>: the first
	 *       {@value PatternLayerManager#BATCHED_SOURCE_LENGTH} samples of the
	 *       innermost-cached-resampled note audio, copied into a freshly
	 *       allocated buffer.</li>
	 *   <li><b>Pitch ratio</b>: {@code 1.0} — the source is already at target
	 *       pitch (the {@link org.almostrealism.audio.notes.NoteAudioProvider}
	 *       cache stores audio resampled to the {@link KeyPosition}'s
	 *       frequency), so the batched resample kernel passes through the first
	 *       {@code targetLength} samples unchanged.</li>
	 *   <li><b>Filter cutoff envelope</b>: per-sample curve generated by
	 *       {@link ParameterizedFilterEnvelope.Filter#writeCutoffBuffer} from
	 *       the chain's filter envelope ADSR parameters and the live automation
	 *       level.</li>
	 *   <li><b>Volume envelope</b>: per-sample curve generated by
	 *       {@link ParameterizedVolumeEnvelope.Filter#writeEnvelopeBuffer} from
	 *       the chain's volume envelope ADSR parameters and the live automation
	 *       level.</li>
	 * </ul>
	 *
	 * <p>Notes without both a {@link ParameterizedFilterEnvelope.Filter} and a
	 * {@link ParameterizedVolumeEnvelope.Filter} wrapper in their delegate chain
	 * (e.g. non-melodic / percussive elements built without envelopes) leave the
	 * batched fields {@code null} and dispatch through the per-note path via
	 * the renderer's documented fallback behaviour. Exceptions during extraction
	 * are caught: the batched fields are left {@code null} and the per-note
	 * fallback handles the note correctly.</p>
	 *
	 * @param note              the rendered-note audio whose batched fields to populate
	 * @param element           the originating pattern element
	 * @param details           voicing details for this note
	 * @param automationLevel   automation factor producer for the active time window
	 * @param audioContext      note audio context
	 * @param durationSec       effective note duration in seconds
	 */
	private void populateBatchedInputs(RenderedNoteAudio note,
									   ...
									   double durationSec) {
		try {
			PatternNote patternNote = element.getNote(details.getVoicing());
			if (patternNote == null) return;

			ParameterizedVolumeEnvelope.Filter volFilter =
					patternNote.findFilter(ParameterizedVolumeEnvelope.Filter.class);
			ParameterizedFilterEnvelope.Filter filtFilter =
					patternNote.findFilter(ParameterizedFilterEnvelope.Filter.class);
			if (volFilter == null || filtFilter == null) return;

			int sourceLength = PatternLayerManager.BATCHED_SOURCE_LENGTH;
			int targetLength = PatternLayerManager.BATCHED_TARGET_LENGTH;
			int sampleRate = OutputLine.sampleRate;

			PatternNoteAudio leaf = patternNote.innermostAudio();
			if (leaf == null) return;
			Producer<PackedCollection> leafAudioProducer = leaf.getAudio(
					details.getTarget(), details.getStereoChannel().getIndex(),
					audioContext.getAudioSelection());
			// Materialise the leaf audio source. The .evaluate() lives on
			// PatternFeatures (an EVALUATE_ALLOWED_FILES boundary); for the
			// NoteAudioProvider leaf the result is a cache hit on every call
			// after the first one for the same (target, channel).
			PackedCollection raw = PatternFeatures.materialiseProducer(leafAudioProducer);
			if (raw == null || raw.getMemLength() == 0) return;

			PackedCollection src = new PackedCollection(sourceLength);
			int copy = Math.min(raw.getMemLength(), sourceLength);
			src.setMem(0, raw, 0, copy);

			// Read the automation level scalar at note start-time. Same
			// .evaluate() boundary trick — the materialisation runs on
			// PatternFeatures while the .toDouble(0) extraction runs here
			// (ScaleTraversalStrategy is on TODOUBLE_ALLOWED_FILES). The
			// scalar feeds the same {@code adjustmentBase + adjustmentAutomation * al}
			// computation used by the per-note Filter.apply path so the batched
			// dispatch preserves the acoustic envelope shape exactly.
			double autoLevel = 1.0;
			try {
				PackedCollection autoSample = PatternFeatures.materialiseProducer(
						automationLevel.getResultant(c(0.0)));
				if (autoSample != null && autoSample.getMemLength() > 0) {
					autoLevel = autoSample.toDouble(0);
				}
			} catch (Exception ignored) {
				// Default automation level — envelope shape only modestly affected
			}

			PackedCollection volEnv = new PackedCollection(targetLength);
			volFilter.writeEnvelopeBuffer(volEnv, sampleRate, durationSec, autoLevel);

			PackedCollection cutEnv = new PackedCollection(targetLength);
			filtFilter.writeCutoffBuffer(cutEnv, sampleRate, durationSec, autoLevel);

			note.setBatchedSource(src);
			note.setBatchedPitchRatio(1.0);
			note.setBatchedFilterCutoffEnvelope(cutEnv);
			note.setBatchedVolumeEnvelope(volEnv);
		} catch (Exception e) {
			// Population failed — leave batched fields null. The
			// BatchedPatternLayerRenderer will fall back to the per-note path
			// or fail loud in strict mode.
		}
	}
