...
public class SampleBrushVisualTest extends TestSuiteBase {
	...
	private WaveDetails createSourcePattern() {
		...
		// Low band: bins around 25-35, dots every 50 frames; mid band: bins around
		// 115-125, dots every 100 frames; high band: bins around 200-210, dots every
		// 150 frames -- the whole canvas is generated by one device computation
		bandDots(30, 50, dotMagnitude)
				.add(bandDots(120, 100, dotMagnitude))
				.add(bandDots(205, 150, dotMagnitude))
				.into(freqData.traverseEach()).evaluate();
	...
	/**
	 * Produces the dot pattern for one frequency band as a single computation over the
	 * whole canvas: a cell {@code (frame, bin)} carries the magnitude when
	 * {@code frame % spacing} is within the four-frame dot length and the bin is within
	 * one of the band's center bin, and zero everywhere else. The pattern is a pure
	 * function of the cell index, so no host-computed value ever reaches the device.
	 *
	 * @param bin       the band's center frequency bin
	 * @param spacing   the number of frames between dots
	 * @param magnitude the magnitude painted inside each dot
	 * @return the band's dot pattern over the full canvas
	 */
	private CollectionProducer bandDots(int bin, int spacing, double magnitude) {
		int total = FREQ_FRAMES * FREQ_BINS;
		CollectionProducer frame = floor(integers(0, total).divide((double) FREQ_BINS));
		CollectionProducer binIndex = integers(0, total).mod((double) FREQ_BINS);
		CollectionProducer inDot = lessThan(frame.mod((double) spacing), c(4.0), c(1.0), c(0.0));
		CollectionProducer inBand = lessThan(abs(binIndex.add(c((double) -bin))), c(1.5), c(1.0), c(0.0));
		return inDot.multiply(inBand).multiply(magnitude);
	}
