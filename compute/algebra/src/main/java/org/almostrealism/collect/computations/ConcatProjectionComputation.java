...
public class ConcatProjectionComputation extends TraversableExpressionComputation {
	...
	/** Shape of the differentiation target. */
	private final TraversalPolicy targetShape;
	/** Axis along which the inputs were concatenated. */
	private final int axis;
	/** Cumulative boundary positions of each input segment along the concatenation axis. */
	private final int[] segmentBoundaries;
	/** Per-segment position offsets into the target (one array of offsets per input). */
	private final Expression<?>[][] segmentOffsets;
	/** Scale factor applied to each input segment's contribution (typically ±1). */
	 ...
	 * {@code idx / targetSize}, target index {@code idx % targetSize}, mapped through
	 * the matching concat segment). An absolute, input-free expression is correct at
	 * any kernel width, so it never needs per-element unrolling. Returning
	 * {@link #getMemLength()} keeps the emitted scope a single
	 * {@link io.almostrealism.kernel.KernelIndex} parameterized statement evaluated in
	 * parallel across all {@link #getCountLong()} Jacobian entries at kernel runtime,
	 * rather than expanding the entire dense Jacobian at compile time. Mirrors the same
	 * override on {@link SubsetProjectionComputation}.</p>
	 *
