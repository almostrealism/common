...
/**
 * A temporal cell that generates a periodic waveform with configurable frequency,
 * amplitude, phase, and envelope. This class owns everything the oscillator cells
 * have in common — parameter storage, the initial-value setup, the runtime setters,
 * the envelope and the per-sample position advancement — and leaves only the
 * waveform itself to subclasses via {@link #waveform(CollectionProducer)}.
 *
 ...
 * @see Factor
 */
public abstract class OscillatorCell extends CollectionTemporalCellAdapter implements SamplingFeatures {
	...
	/**
	 * Creates a new oscillator with the specified data storage and an initial
	 * amplitude of 1.0.
	 *
	...
	/**
	 * Returns the hardware-side data storage for the wave parameters.
	 *
	 * @return the data storage this oscillator reads and writes
	 */
	protected SineWaveCellData getData() { return data; }
	...
	public Supplier<Runnable> setup() {
		OperationList defaults = new OperationList(getClass().getSimpleName() + " Default Value Assignment");
		...
		OperationList setup = new OperationList(getClass().getSimpleName() + " Setup");
		setup.add(defaults);
	 ...
	 * <p>
	 * The output is computed as {@code envelope * amplitude * waveform * depth}, where
	 * the waveform is produced by {@link #waveform(CollectionProducer)} from the raw
	 * phase {@code wavePosition + phase}.
	 * </p>
	 * <p>
	...
	public Supplier<Runnable> push(Producer<PackedCollection> protein) {
		...
		OperationList push = new OperationList(getClass().getSimpleName() + " Push");

		Producer<PackedCollection> envelope = env == null ? scalar(1.0) :
				env.getResultant(data.getNotePosition());

		CollectionProducer phase = add(data.getWavePosition(), data.getPhase());
		CollectionProducer result = multiply(multiply(envelope, data.getAmplitude()),
				multiply(waveform(phase), data.getDepth()));
	...
	/**
	 * Builds the waveform for the given raw phase. The phase is measured in cycles
	 * and is unbounded (it is {@code wavePosition + phase offset}); use
	 * {@link #cyclePosition(CollectionProducer)} for the position within the cycle.
	 *
	 * @param phase the raw phase in cycles
	 * @return the waveform value, nominally in the range -1 to +1
	 */
	protected abstract CollectionProducer waveform(CollectionProducer phase);

	/**
	 * Reduces a raw phase to its position within the current cycle,
	 * {@code phase - floor(phase)}, in the range 0 (inclusive) to 1 (exclusive).
	 *
	 * @param phase the raw phase in cycles
	 * @return the fractional cycle position
	 */
	protected CollectionProducer cyclePosition(CollectionProducer phase) {
		return subtract(phase, floor(phase));
	}
	...
	public Supplier<Runnable> tick() {
		OperationList tick = new OperationList(getClass().getSimpleName() + " Tick");
