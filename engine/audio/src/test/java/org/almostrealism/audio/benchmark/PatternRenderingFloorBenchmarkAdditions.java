...
/**
 * Additional pattern rendering floor benchmarks beyond the primary test.
 * All tests in this class use the same constants and helper methods as
 * the primary benchmark, inherited from {@link PatternRenderingFloorBenchmarkBase}.
 */
public class PatternRenderingFloorBenchmarkAdditions extends PatternRenderingFloorBenchmarkBase {
	// ===========================================================================================
	...
	/**
	 * Builds and compiles the resample-only kernel: linear resample from
	 * {@code [SOURCE_SIZE]} to {@code [NOTE_SIZE]} via precomputed fractional indices.
	 */
	...
	/**
	 * Builds and compiles the volume-envelope-only kernel: element-wise multiply of
	 * pre-resampled audio by an ADSR envelope.
	 */
	...
	/**
	 * Builds and compiles the FIR lowpass-only kernel: applies the precomputed
	 * {@code lpCoeffs} FIR filter to the volume-enveloped audio.
	 */
	...
	/**
	 * Builds and compiles the accumulate-only kernel: element-wise add of filtered
	 * note audio into an accumulation buffer.
	 */
	...
	/**
	 * Logs per-phase timing statistics for one kernel: mean, median, p95, and
	 * amortized per-note cost in milliseconds.
	 */
	...
	/**
	 * Formats a single row of the per-phase summary table showing kernel name,
	 * mean time in ms, and percentage of total.
	 */
	...
	public void benchmarkBatchedChainWithPaddedFir() throws Exception {
		...
		for (int d = 0; d < NOTES_PER_MEASURE_VALUES.length; d++) {
			...
			try {
				...
				log("Unable to compile batched padded FIR chain at " + totalNotes + " notes: "
	...
	/**
	 * Logs timing statistics for a batched (single {@code evaluate()} per density)
	 * measurement, including amortized per-note cost and batching speedup vs the
	 * sequential per-note baseline.
	 */
	...
	public void benchmarkFilterEnvelopeBatched() throws Exception {
		...
		try {
			...
			log("Unable to compile sequential per-note lowPass with per-sample cutoff: "
					+ ex.getClass().getSimpleName() + ": " + ex.getMessage());
		...
		for (int d = 0; d < NOTES_PER_MEASURE_VALUES.length; d++) {
			...
			try {
				...
				log("Unable to compile batched filter envelope at " + totalNotes + " notes: "
						+ ex.getClass().getSimpleName() + ": " + ex.getMessage());
	...
	public void benchmarkCombinedEnvelopeChainBatched() throws Exception {
		...
		try {
			...
			log("Unable to compile sequential combined chain: "
					+ ex.getClass().getSimpleName() + ": " + ex.getMessage());
			log("Skipping E3.");
			log("==========================================================");
			return;
		...
		for (int d = 0; d < NOTES_PER_MEASURE_VALUES.length; d++) {
			...
			try {
				...
				log("Unable to compile batched combined chain at " + totalNotes + " notes: "
						+ ex.getClass().getSimpleName() + ": " + ex.getMessage());
				log("");
				continue;
			}
	...
	private static final class B1NoteMetadata {
		/** Source buffer index for each note into the source pool. */
		final int[] sourceIdx;
		...
		/** Volume envelope attack time fraction for each note. */
		final double[] volAttack;

		/** Volume envelope decay time fraction for each note. */
		final double[] volDecay;

		/** Volume envelope sustain level for each note. */
		final double[] volSustain;

		/** Volume envelope release time fraction for each note. */
		final double[] volRelease;

		/** Filter envelope attack time fraction for each note. */
		final double[] filterAttack;

		/** Filter envelope decay time fraction for each note. */
		final double[] filterDecay;

		/** Filter envelope sustain level for each note. */
		final double[] filterSustain;

		/** Filter envelope release time fraction for each note. */
		final double[] filterRelease;

		/** Automation level (0-1) for each note. */
		final double[] automationLevel;

		/** Tick-relative start sample offset for each note. */
		final double[] tickStartOffset;

		/**
		 * Constructs metadata arrays for {@code totalNotes} notes, initially populated
		 * with zero/empty values. Callers populate via {@link #buildB1NoteMetadata}.
		 */
	...
	private static final class B1Chain {
		/** Pre-allocated audio input buffer captured by reference in the chain. */
		final PackedCollection audioInput;

		/** Compiled E3-style batched chain operating on the audio input buffer. */
		final Evaluable<PackedCollection> chain;

		/**
		 * Constructs a B1Chain with the given audio input buffer and compiled chain.
		 *
		 * @param audioInput pre-allocated {@code [N, paddedNoteSize]} audio buffer
		 * @param chain compiled E3-style chain that reads from {@code audioInput}
		 */
	...
	public void benchmarkJavaSideGatherCostB1() throws Exception {
		...
		for (int npm : NOTES_PER_MEASURE_VALUES) {
			...
			try {
				...
				log("Unable to compile B1+E3 chain at " + totalNotes + " notes: "
						+ ex.getClass().getSimpleName() + ": " + ex.getMessage());
				log("");
				continue;
			}
