 ...
 */
public interface Bottleneck extends LayerFeatures {
	 ...
	 */
	default Block decode(int batchSize, int seqLength) {
		TraversalPolicy shape = shape(batchSize, getOutputDim(), seqLength);
		return layer("bottleneckDecodeIdentity", shape, shape, input -> c(input));
	}
