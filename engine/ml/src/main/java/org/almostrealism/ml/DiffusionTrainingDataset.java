...
public class DiffusionTrainingDataset implements Dataset<PackedCollection> {
	...
	/**
	 * Every timestep the scheduler can sample, normalized to {@code t / numSteps}.
	 *
	 * <p>The whole schedule is generated by the computation graph once, so producing the
	 * timestep for a sample is a copy out of this table rather than a value carried over
	 * from the host.</p>
	 */
	private final PackedCollection normalizedTimesteps;
	...
	public DiffusionTrainingDataset(List<PackedCollection> samples,
		...
		int numSteps = scheduler.getNumSteps();
		this.normalizedTimesteps = CollectionFeatures.getInstance()
				.integers(0, numSteps).divide(numSteps).evaluate();
	}
