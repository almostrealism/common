/*
 * Copyright 2025 Michael Murray
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *    http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package org.almostrealism.layers.test;

import org.almostrealism.collect.PackedCollection;
import org.almostrealism.io.Console;
import org.almostrealism.io.OutputFeatures;
import org.almostrealism.io.SystemUtils;
import org.almostrealism.model.CompiledModel;
import org.almostrealism.model.Model;
import org.almostrealism.util.TestSuiteBase;
import org.junit.Test;

import java.io.IOException;
import java.nio.file.Path;
import java.nio.file.Paths;
import java.util.Arrays;

/**
 * Tests ConvTranspose1d against PyTorch reference outputs.
 *
 * <p>Reference data generated by scripts/generate-conv-transpose-reference.py which creates test cases
 * with known sequential input values for easy verification.</p>
 *
 * <p>This test isolates ConvTranspose1d indexing issues by using simple inputs
 * that make manual verification straightforward.</p>
 */
public class ConvTranspose1dReferenceTest extends TestSuiteBase {

	/**
	 * Base directory for reference test data. The data is generated binary content, so it
	 * lives outside the repository working tree: {@code $AR_TEST_DATA/conv_transpose} when
	 * {@code AR_TEST_DATA} is set, otherwise {@code ~/.ar/test_data/conv_transpose}.
	 */
	private static final Path BASE_DIR = SystemUtils.getProperty("AR_TEST_DATA") == null
			? Paths.get(SystemUtils.getHome(), ".ar", "test_data", "conv_transpose")
			: Paths.get(SystemUtils.getProperty("AR_TEST_DATA"), "conv_transpose");
	/** Tolerance for floating-point comparisons. */
	private static final double TOLERANCE = 0.001;

	/**
	 * Loads a reference file written in the native {@link PackedCollection} serialized
	 * format (shape header followed by values; see {@link PackedCollection#loadCollections}).
	 * The shape travels with the file, so no dimensions are supplied here.
	 *
	 * @param testCase Test case directory name
	 * @param filename Name of the reference file
	 * @return The deserialized collection
	 * @throws IOException If the file cannot be read
	 */
	private PackedCollection loadReference(String testCase, String filename) throws IOException {
		Path filepath = BASE_DIR.resolve(testCase).resolve(filename);

		if (!filepath.toFile().exists()) {
			throw new IOException("Reference file not found: " + filepath +
					"\nRun: python3 scripts/generate-conv-transpose-reference.py");
		}

		return PackedCollection.loadCollections(filepath.toFile()).iterator().next();
	}

	/**
	 * Compares actual output against expected reference values.
	 *
	 * @param testName Name of the test for logging
	 * @param actual Actual output collection
	 * @param expected Expected reference values
	 */
	private void compareOutputs(String testName, PackedCollection actual, PackedCollection expected) {
		int size = (int) actual.getMemLength();
		assertEquals(testName + " size mismatch", (int) expected.getMemLength(), size);

		log("\n" + testName + " comparison:");
		double maxDiff = 0;
		double sumDiff = 0;
		int failCount = 0;

		for (int i = 0; i < size; i++) {
			double actualVal = actual.toDouble(i);
			double expectedVal = expected.toDouble(i);
			double diff = Math.abs(actualVal - expectedVal);

			maxDiff = Math.max(maxDiff, diff);
			sumDiff += diff;

			if (diff > TOLERANCE) {
				failCount++;
				if (failCount <= 10) {
					log(String.format("  [%d] actual=%.6f, expected=%.6f, diff=%.6f FAIL",
							i, actualVal, expectedVal, diff));
				}
			} else if (i < 5) {
				log(String.format("  [%d] actual=%.6f, expected=%.6f, diff=%.6f OK",
						i, actualVal, expectedVal, diff));
			}
		}

		double mae = sumDiff / size;
		log(String.format("\nMAE: %.6f, MaxDiff: %.6f, Failures: %d/%d",
				mae, maxDiff, failCount, size));

		assertEquals(testName + " has failures", 0, failCount);
	}

	/**
	 * Minimal test: 1 input channel, 1 output channel, stride=2, no padding.
	 * Input: [1, 2] -> Output: [0.01, 0.02, 0.02, 0.04]
	 */
	@Test(timeout = 60000)
	public void testMinimal() throws IOException {
		if (!BASE_DIR.resolve("minimal").toFile().exists()) {
			log("Skipping - reference data not found. Run scripts/generate-conv-transpose-reference.py");
			return;
		}

		Console.root().addListener(OutputFeatures.fileOutput(
				BASE_DIR.resolve("test_minimal.log").toString()));

		log("=== ConvTranspose1d Minimal Test ===");

		PackedCollection inputC = loadReference("minimal", "input.bin");
		PackedCollection weightsC = loadReference("minimal", "weights.bin");
		PackedCollection biasC = loadReference("minimal", "bias.bin");
		PackedCollection expected = loadReference("minimal", "expected_output.bin");

		log("Input: " + Arrays.toString(inputC.toArray()));
		log("Weights: " + Arrays.toString(weightsC.toArray()));
		log("Bias: " + Arrays.toString(biasC.toArray()));
		log("Expected output: " + Arrays.toString(expected.toArray()));

		// Parameters: batch=1, in_ch=1, out_ch=1, seq=2, kernel=2, stride=2, padding=0
		int batchSize = 1;
		int inChannels = 1;
		int outChannels = 1;
		int seqLength = 2;
		int kernel = 2;
		int stride = 2;
		int padding = 0;
		int outputPadding = 0;

		Model model = new Model(shape(batchSize, inChannels, seqLength));
		model.add(convTranspose1d(batchSize, inChannels, outChannels, seqLength,
				kernel, stride, padding, outputPadding, weightsC, biasC));

		CompiledModel compiled = model.compile(false);
		PackedCollection output = compiled.forward(inputC);

		log("Output shape: " + output.getShape());
		compareOutputs("minimal", output, expected);

		log("\n=== Minimal Test PASSED ===");
	}

	/**
	 * Test with padding: kernel=4, stride=2, padding=1.
	 */
	@Test(timeout = 60000)
	public void testWithPadding() throws IOException {
		if (!BASE_DIR.resolve("with_padding").toFile().exists()) {
			log("Skipping - reference data not found. Run scripts/generate-conv-transpose-reference.py");
			return;
		}

		Console.root().addListener(OutputFeatures.fileOutput(
				BASE_DIR.resolve("test_with_padding.log").toString()));

		log("=== ConvTranspose1d With Padding Test ===");

		PackedCollection inputC = loadReference("with_padding", "input.bin");
		PackedCollection weightsC = loadReference("with_padding", "weights.bin");
		PackedCollection biasC = loadReference("with_padding", "bias.bin");
		PackedCollection expected = loadReference("with_padding", "expected_output.bin");

		log("Input: " + Arrays.toString(inputC.toArray()));
		log("Weights: " + Arrays.toString(weightsC.toArray()));
		log("Expected output: " + Arrays.toString(expected.toArray()));

		int batchSize = 1;
		int inChannels = 1;
		int outChannels = 1;
		int seqLength = 2;
		int kernel = 4;
		int stride = 2;
		int padding = 1;
		int outputPadding = 0;

		Model model = new Model(shape(batchSize, inChannels, seqLength));
		model.add(convTranspose1d(batchSize, inChannels, outChannels, seqLength,
				kernel, stride, padding, outputPadding, weightsC, biasC));

		CompiledModel compiled = model.compile(false);
		PackedCollection output = compiled.forward(inputC);

		log("Output shape: " + output.getShape());
		compareOutputs("with_padding", output, expected);

		log("\n=== With Padding Test PASSED ===");
	}

	/**
	 * Multi-channel test: in_ch=2, out_ch=2, stride=2.
	 */
	@Test(timeout = 60000)
	public void testMultichannelSmall() throws IOException {
		if (!BASE_DIR.resolve("multichannel_small").toFile().exists()) {
			log("Skipping - reference data not found. Run scripts/generate-conv-transpose-reference.py");
			return;
		}

		Console.root().addListener(OutputFeatures.fileOutput(
				BASE_DIR.resolve("test_multichannel_small.log").toString()));

		log("=== ConvTranspose1d Multichannel Small Test ===");

		PackedCollection inputC = loadReference("multichannel_small", "input.bin");
		PackedCollection weightsC = loadReference("multichannel_small", "weights.bin");
		PackedCollection biasC = loadReference("multichannel_small", "bias.bin");
		PackedCollection expected = loadReference("multichannel_small", "expected_output.bin");

		double[] input = inputC.toArray();
		double[] weights = weightsC.toArray();
		log("Input (first 10): " + Arrays.toString(Arrays.copyOf(input, Math.min(10, input.length))));
		log("Weights (first 10): " + Arrays.toString(Arrays.copyOf(weights, Math.min(10, weights.length))));

		int batchSize = 1;
		int inChannels = 2;
		int outChannels = 2;
		int seqLength = 2;
		int kernel = 2;
		int stride = 2;
		int padding = 0;
		int outputPadding = 0;

		Model model = new Model(shape(batchSize, inChannels, seqLength));
		model.add(convTranspose1d(batchSize, inChannels, outChannels, seqLength,
				kernel, stride, padding, outputPadding, weightsC, biasC));

		CompiledModel compiled = model.compile(false);
		PackedCollection output = compiled.forward(inputC);

		log("Output shape: " + output.getShape());
		compareOutputs("multichannel_small", output, expected);

		log("\n=== Multichannel Small Test PASSED ===");
	}

	/**
	 * Oobleck-like test: stride=4, outputPadding=3.
	 * Similar to real Oobleck decoder but with tiny dimensions.
	 */
	@Test(timeout = 60000)
	public void testOobleckLike() throws IOException {
		if (!BASE_DIR.resolve("oobleck_like").toFile().exists()) {
			log("Skipping - reference data not found. Run scripts/generate-conv-transpose-reference.py");
			return;
		}

		Console.root().addListener(OutputFeatures.fileOutput(
				BASE_DIR.resolve("test_oobleck_like.log").toString()));

		log("=== ConvTranspose1d Oobleck-Like Test ===");

		PackedCollection inputC = loadReference("oobleck_like", "input.bin");
		PackedCollection weightsC = loadReference("oobleck_like", "weights.bin");
		PackedCollection biasC = loadReference("oobleck_like", "bias.bin");
		PackedCollection expected = loadReference("oobleck_like", "expected_output.bin");

		log("Input: " + Arrays.toString(inputC.toArray()));
		log("Expected output: " + Arrays.toString(expected.toArray()));

		int batchSize = 1;
		int inChannels = 4;
		int outChannels = 2;
		int seqLength = 2;
		int kernel = 4;
		int stride = 4;
		int padding = 1;
		int outputPadding = 3;

		Model model = new Model(shape(batchSize, inChannels, seqLength));
		model.add(convTranspose1d(batchSize, inChannels, outChannels, seqLength,
				kernel, stride, padding, outputPadding, weightsC, biasC));

		CompiledModel compiled = model.compile(false);
		PackedCollection output = compiled.forward(inputC);

		log("Output shape: " + output.getShape());
		compareOutputs("oobleck_like", output, expected);

		log("\n=== Oobleck-Like Test PASSED ===");
	}

	/**
	 * Stride=16 test: exactly like Oobleck decoder block 1 parameters but tiny channels.
	 * in_ch=2, out_ch=1, seq=2, kernel=16, stride=16, padding=7, outputPadding=15.
	 * Output length = (2-1)*16 - 2*7 + 16 + 15 = 16 - 14 + 31 = 33.
	 */
	@Test(timeout = 60000)
	public void testStride16Tiny() throws IOException {
		if (!BASE_DIR.resolve("stride16_tiny").toFile().exists()) {
			log("Skipping - reference data not found. Run scripts/generate-conv-transpose-reference.py");
			return;
		}

		Console.root().addListener(OutputFeatures.fileOutput(
				BASE_DIR.resolve("test_stride16_tiny.log").toString()));

		log("=== ConvTranspose1d Stride=16 Tiny Test ===");

		PackedCollection inputC = loadReference("stride16_tiny", "input.bin");
		PackedCollection weightsC = loadReference("stride16_tiny", "weights.bin");
		PackedCollection biasC = loadReference("stride16_tiny", "bias.bin");
		PackedCollection expected = loadReference("stride16_tiny", "expected_output.bin");

		log("Input: " + Arrays.toString(inputC.toArray()));
		log("Weights (first 16): " + Arrays.toString(Arrays.copyOf(weightsC.toArray(), 16)));
		log("Expected output (first 16): " + Arrays.toString(Arrays.copyOf(expected.toArray(), 16)));

		int batchSize = 1;
		int inChannels = 2;
		int outChannels = 1;
		int seqLength = 2;
		int kernel = 16;
		int stride = 16;
		int padding = 7;
		int outputPadding = 15;

		Model model = new Model(shape(batchSize, inChannels, seqLength));
		model.add(convTranspose1d(batchSize, inChannels, outChannels, seqLength,
				kernel, stride, padding, outputPadding, weightsC, biasC));

		CompiledModel compiled = model.compile(false);
		PackedCollection output = compiled.forward(inputC);

		log("Output shape: " + output.getShape());
		log("Output values (first 16):");
		for (int i = 0; i < Math.min(16, output.getMemLength()); i++) {
			log(String.format("  [%d] = %.6f", i, output.toDouble(i)));
		}

		compareOutputs("stride16_tiny", output, expected);

		log("\n=== Stride=16 Tiny Test PASSED ===");
	}
}
