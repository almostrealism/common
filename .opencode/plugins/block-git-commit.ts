...
//
// opencode adapter for the "block git commit" policy.
//
// The decision logic lives in the shared core
//   .claude/hooks/lib/git_command_check.py
// (policy: block-git-commit). This plugin is the opencode counterpart
// to .claude/hooks/block-git-commit.sh: it pulls the bash command out
// of the opencode tool-call event, asks the shared core to decide,
// and translates the structured result into the opencode-native
