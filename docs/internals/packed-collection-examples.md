...
three or fewer values are excluded because they are not a kernel's worth of
work, and small fixed-size types such as `Pair` (two values) and `Vector`
(three values) are filled this way in the ordinary course of things. The
report names the collection and the cost, which is enough to know a call is
worth replacing. To also see *where* the call came from, set
`AR_FILL_ORIGIN=true` (or `PackedCollection.enableFillOrigin = true`) — each
report then names its origin in the message rather than only in the stack, so
repeated warnings remain distinguishable after the message-deduplicator drops
them.
