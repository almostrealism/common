...
/**
 * Runtime-compiled JNI operation that flushes a shared-memory mapping back to its file via
...
class SharedMemorySync extends BaseNative {
	 ...
	 */
	SharedMemorySync(NativeCompiler compiler) {
		super(compiler);
		initNative();
	}
	...
	public String getFunctionDefinition() {
		return "#include <sys/mman.h>\n" +
				...
				"\tif (sharedMemory) msync(sharedMemory, length, MS_SYNC);\n" +
				"}\n";
	}

	/**
	 * Flushes pending changes in the shared-memory buffer back to the underlying region.
	 *
	 * @param buffer the mapped {@link ByteBuffer}
	 * @param length the number of bytes to sync
	 */
