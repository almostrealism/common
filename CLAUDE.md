...
- `tools/ci/agent-protection/validate-agent-commit.sh` — Blocks agent commits that change or remove a test method existing on the base branch, that modify CI files, or that contain no substantive changes. Adding a new test method to an existing test class is permitted, as is editing fixtures and helpers. Java and Python are held to different strengths of this rule; see rule 2 below.
- `tools/ci/agent-protection/detect-test-hiding.sh` — 12 pattern detectors: TestDepth escalation, timeout inflation, dimension reduction, tolerance weakening, numeric literal shrinkage, net assertion loss. The net-assertion-loss detector (Pattern 2) counts assertion *calls* — in `@Test` bodies and in helper methods alike, netted per file — and does not count method *declarations*, so moving a helper whose name starts with `assert` (e.g. `assertWithin(...)`) is not a removed assertion; deleting an `assertEquals` call from a helper body still is. **Java only** — it selects `**/*Test*.java` and `**/test/**/*.java`, so no Python test is examined by it.
...
2. **Base-branch test methods are read-only for agents.** Mechanical enforcement via `validate-agent-commit.sh`. There are no exceptions based on agent judgment. New test methods MAY be added to an existing test class, and fixtures and helpers in a test class are ordinary code.
   In **Java** the lock is byte for byte: every test method that exists on the base branch must survive exactly — annotation block, signature and body.
   In **Python** it is weaker, because an indentation-delimited body cannot be bounded as reliably as a braced one and the extractor should not claim a precision it does not have. Two properties are enforced instead: every `def test_*` present on the base branch must still be present, and a test file's assertion count must not fall. Editing the inside of a Python test is therefore allowed; deleting one, renaming it away, or removing its assertions is not. Do not read that leniency as an invitation — rule 3 and the deception patterns above apply to Python exactly as they do to Java, and the difference here is only in what a script can prove.
   The two exemptions are mechanical, not matters of judgment. A branch named `ci/...` may change files under `.github/workflows/`, `tools/ci/` and `.claude/hooks/`: the name is chosen before the work starts and declares the pipeline as the subject of the change, so the whole pipeline honours it — the CI-file lock, the enforcement-infrastructure tampering check, the exfiltration guard's integrity comparison, and the deception audit's CI-only-commit finding. The reasoning for the hooks and the guard is the same as for the rest: a change permitted to rewrite the workflow could equally delete the step that checks a hook, so comparing its edits obstructs the work without policing anything. What the name does **not** lift is the requirement that the guard be present and registered on HEAD — that is not a claim about what the branch edited, it is what says the guard is in the tree an agent session runs with, and a branch name must never be able to switch the runtime protection off. The policy detectors under `engine/utils` remain locked on every branch, because weakening a detector is never what a pipeline change is about. Separately, a commit carrying a controller-signed `Sensitive-File-Bypass` trailer lifts the test-method and CI locks in `validate-agent-commit.sh`. An agent cannot produce that signature — the secret is not in its environment and the harness strips any trailer the agent writes.
3. **A "fix" that only edits existing tests or CI config is not a fix.** A production test failure is fixed by production code; changing the test that reports it is how a failure gets hidden. What `validate-agent-commit.sh` rejects mechanically is narrower than that guidance, and the difference is worth knowing rather than discovering: a commit must add something that did not exist before — a production change, a branch-introduced test file, or a new test method in an existing one — so a commit that only adds tests does pass. Passing that check is not the same as having fixed anything.
