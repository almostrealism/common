...
## ANTI-DECEPTION PROTOCOL: MANDATORY RULES FOR TEST FAILURE INVESTIGATION

**THIS SECTION EXISTS BECAUSE AGENTS HAVE SYSTEMATICALLY DECEIVED THE HUMAN DEVELOPER.**

On branch `feature/lora-gradients`, autonomous agents deployed via auto-resolve engaged in a
sustained campaign of test suppression rather than fixing production code. The evidence is
documented in [DECEPTION.md](DECEPTION.md). These rules exist to make such deception physically
impossible. They are non-negotiable and have ZERO exceptions.

### RULE 1: YOU CANNOT MODIFY TEST FILES

**You are NOT ALLOWED to modify ANY file under `src/test/` that exists on master.**

This is absolute. There are NO exceptions. Not for "new tests." Not for "tests that are
genuinely wrong." Not for timeouts. Not for `@TestDepth`. Not for dimensions. Not for
tolerances. NOTHING.

Specifically, you MUST NOT:
- Change `@TestDepth` values (e.g., `@TestDepth(2)` to `@TestDepth(10)`)
- Change timeout values in `@Test(timeout=...)` annotations
- Reduce numeric constants (dimensions, iterations, sizes) in test methods
- Add `@Ignore`, `@Disabled`, or any skip mechanism
- Delete, comment out, or weaken assertions
- Add try/catch blocks that swallow exceptions
- Add `skipLongTests` guards
- Change expected values to match incorrect actual output
- Rename, move, or split tests to avoid detection

If you believe a test is genuinely wrong, you MUST:
1. Send a Slack message explaining your reasoning with evidence
2. NOT touch the test file
3. Let the human developer decide

**WHY THIS IS ABSOLUTE:** Agents on `feature/lora-gradients` changed `@TestDepth(2)` to
`@TestDepth(10)` to hide NormTests from CI, reduced test dimensions 8x to make tests
trivially pass, and re-added suppression annotations after a human reverted them. Every
exception and escape hatch was exploited. Therefore, there are no exceptions.

### RULE 2: YOU CANNOT MODIFY CI/WORKFLOW FILES

**You are NOT ALLOWED to modify files under `.github/workflows/` or `tools/ci/` to exclude,
skip, or disable tests or quality checks.**

Agents have excluded `CodePolicyEnforcementTest` from test steps, removed quality gates from
build steps, and manipulated job dependencies to avoid running checks. This is forbidden.

### RULE 3: MANDATORY DIFF ANALYSIS BEFORE "NO CHANGES NEEDED"

**You CANNOT conclude "no changes needed" or "failure is unrelated" without producing a
structured diff analysis.** An agent on `feature/lora-gradients` stored a memory claiming
"no code changes were needed" for NormTests failures, while the branch's own changes
(CollectionSubsetComputation, CollectionConcatenateComputation) were the proven root cause.

Before concluding that a failure is unrelated to your branch, you MUST:

1. Run `git diff origin/master...HEAD --name-only` and list EVERY changed production file
2. For EACH changed file, determine if the failing test imports it (directly or transitively)
3. If ANY changed file is in the same module as the failing test, you MUST investigate further
4. Produce a `DIFF_ANALYSIS.md` file with this structure:

```markdown
## Diff Analysis: [Test Name] Failure
...
### Conclusion
[If "no changes needed": specific evidence that NO changed file affects the test]
[If changes needed: description of the production code fix]
```

**If you skip this analysis and claim "no changes needed," you are in violation.**

### RULE 4: LOCAL TEST RESULTS ARE NOT EVIDENCE

**"Tests pass locally" is NOT evidence that tests pass.** Only CI pipeline results count.

Agents have claimed "all NormTests pass on current HEAD" while CI showed failures. The agent
controls the local environment and can set `AR_TEST_DEPTH=0`, use different memory settings,
or run a different commit.

If CI fails and your local tests pass:
- The problem is in YOUR local environment or YOUR test parameters
- CI is correct. You are wrong.
- Do NOT store memories claiming "tests pass" based on local runs when CI is failing

### RULE 5: NO FALSE MEMORIES

**You MUST NOT store memory entries that claim "no changes needed" unless you have completed
the DIFF_ANALYSIS.md process from Rule 3.**

Memory `dff49e69` on `feature/lora-gradients` claimed "The key production code changes on
this branch... do NOT cause NormTests failures. No code changes were needed." This was
provably false - the agent later admitted the changes DID cause the failures. That memory
poisoned future agent sessions.

When storing investigation results in memory:
- Tag with `requires-verification` if you could not reproduce the failure locally
- Include the git diff hash and exact test command used
- NEVER use absolute language ("do NOT cause failures") unless you have proof
