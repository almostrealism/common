...
The rule every branch is held to is the one **`test-integrity-check`** enforces: an existing test may be edited, but it may not be weakened. Its detectors:

- `tools/ci/agent-protection/detect-test-hiding.sh` — Java. 12 pattern detectors: TestDepth escalation, timeout inflation, dimension reduction, tolerance weakening, numeric literal shrinkage, net assertion loss. The net-assertion-loss detector (Pattern 2) counts assertion *calls* — in `@Test` bodies and in helper methods alike, netted per file — and does not count method *declarations*, so moving a helper whose name starts with `assert` (e.g. `assertWithin(...)`) is not a removed assertion; deleting an `assertEquals` call from a helper body still is. It selects `**/*Test*.java` and `**/test/**/*.java` only.
- `tools/ci/agent-protection/detect-python-test-hiding.sh` — Python. Every `def test_*` present on the base branch must still be present, and a test file's assertion count must not fall. An indentation-delimited body cannot be bounded as reliably as a braced one, so these two properties are what the check can honestly prove; editing inside a Python test is otherwise allowed.
- The enforcement-tampering check — the policy detectors, the agent-protection scripts and the exfiltration guard may not change on a PR branch.

Alongside it:

- `tools/ci/agent-protection/check-ci-file-lock.sh` — run by the pipeline's first job, `changes`. Files under `.github/workflows/` and `tools/ci/` change only on a branch named `ci/...` (or under a signed bypass, below); a branch that breaks the lock fails before anything is built.
- `tools/ci/agent-protection/validate-agent-commit.sh` (`agent-commit-validation`) — rejects a change set whose only content is edits to test files that exist on the base branch, with no new test in them. Those tests pass on the base branch, so such a change set cannot fix anything; it can only change what the suite reports.
...
### The per-job test lock

Some jobs are submitted with test-file protection on (`protectTestFiles`): the ones whose premise is that the existing tests are the reference — above all, a job sent to make failing tests pass. For such a job the harness (`TestMethodProtection`, used by `FileStager`) keeps every Java test method that exists on the base branch exactly as it is — annotation block, signature and body — before a commit even exists, and whole-file protects other files under `src/test/`. (CI configuration is protected by the harness for every job, test lock or not, with the same exemptions as the CI-file lock.) New test methods, and edits to fixtures, helpers and tests the branch introduced, are still allowed. It exists because an agent sent to fix a failing test has repeatedly loosened the test instead, which fails `test-integrity-check`, which dispatches an agent to restore it, which fails the test again. It is a constraint of those jobs, not the general rule, and the job's prompt says when it is on.
...
2. **Existing tests may be improved, never weakened.** A test that exists on the base branch is the specification. Editing one — tightening it, fixing a wrong expectation, updating it for an intended change in behaviour — is allowed; making it test less is not: deleting it, disabling or skipping it, removing assertions, loosening tolerances, shrinking dimensions, inflating timeouts or raising `@TestDepth` past what CI runs. `test-integrity-check` enforces this for every branch, and there are no exceptions based on agent judgment. If your job runs with the per-job test lock, existing test methods are read-only in full for that job.
   The exemptions are mechanical, not matters of judgment. A branch named `ci/...` may change files under `.github/workflows/`, `tools/ci/` and `.claude/hooks/`: the name is chosen before the work starts and declares the pipeline as the subject of the change, so the whole pipeline honours it — the CI-file lock, the enforcement-infrastructure tampering check, the exfiltration guard's integrity comparison, and the deception audit's CI-only-commit finding. The reasoning for the hooks and the guard is the same as for the rest: a change permitted to rewrite the workflow could equally delete the step that checks a hook, so comparing its edits obstructs the work without policing anything. What the name does **not** lift is the requirement that the guard be present and registered on HEAD — that is not a claim about what the branch edited, it is what says the guard is in the tree an agent session runs with, and a branch name must never be able to switch the runtime protection off. The policy detectors under `engine/utils` remain locked on every branch, because weakening a detector is never what a pipeline change is about. Separately, a commit carrying a controller-signed `Sensitive-File-Bypass` trailer lifts the CI-file lock. An agent cannot produce that signature — the secret is not in its environment and the harness strips any trailer the agent writes.
3. **A "fix" that only edits existing tests is not a fix.** A production test failure is fixed by production code; changing the test that reports it is how a failure gets hidden. What `agent-commit-validation` rejects mechanically is narrower than that guidance, and the difference is worth knowing rather than discovering: it rejects only a change set made entirely of edits to base-branch test files that adds no new test, so a change set that also touches production code, or adds a test, passes it. Passing that check is not the same as having fixed anything.
